...
export default function ArtworkViewer({ artistName, slides, initialIndex = 0, baseHref = "" }) {
  const [i, setI] = useState(Math.max(0, Math.min(initialIndex, slides.length - 1)));
  ...
  return (
    <section className={styles.wrap} style={infoTop != null ? { '--info-top': `${infoTop}px` } : undefined}>
      {/* Left fixed info */}
      <aside className={styles.info}>
        <div className={styles.artist}>{artistName}</div>
        <div className={styles.workTitle}>{meta.title}{meta.year ? `, ${meta.year}` : ""}</div>
      ...
      <div className={styles.stage}>
        <div className={styles.artBox} ref={artBoxRef}>
          <img src={slide.url} alt={meta.title || "Artwork"} className={styles.art} />
        </div>
      </div>

      {/* Fixed side arrows */}
      <button className={`${styles.arrow} ${styles.left}`} onClick={() => go(-1)} aria-label="Prev">◀</button>
      <button className={`${styles.arrow} ${styles.right}`} onClick={() => go(1)} aria-label="Next">▶</button>
