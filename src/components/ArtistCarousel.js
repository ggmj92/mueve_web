...
export default function ArtistCarousel({ slides, interval = 7000 }) {
    ...
    return (
        <section className={s.stage} aria-label="Artist artworks">
            {/* image area */}
            <div
                className={`${s.frame} ${isPortrait ? s.portrait : s.landscape}`}
            >
            ...
            <div className={s.meta}>
                <div className={s.metaInner}>
                    <span className={s.title}><strong>{slide.title}</strong>{slide.year ? `, ${slide.year}` : ""}</span>
                ...
                <div className={s.controls}>
                    ...
                    <span className={s.idx}>{i + 1} / {len}</span>
